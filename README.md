...
- **Telegram-уведомления**: экран «Интеграции» — бот от @BotFather, `chat_id`, выбор сегментов; воркер шлёт лид в чат сразу после скоринга. Токен хранится в БД (таблица `integrations`), env-переменных не требует.
